...
import { productTypeActions } from "../../redux/actions/productTypeActions";
import {
  Button,
  Card,
  CardBody,
      ...
      {(props) => (
        <div>
          <CardHeader>
            ...
            <CardTitle tag="h4">{prop.tableData.length} Types Listed</CardTitle>
          </CardHeader>
          <CardBody>
            <BootstrapTable
              responsive={true}
...
class ManufacturerSettings extends React.Component {
  ...
  getProductTypes = () => {
    this.props.getProductTypes().then((types) => {
      console.log(types)
      if (types.type && types.type.status == 200) {
        this.setState({
          types: types.type.data,
          total: types.type.data.length,
          totaltypes: types.type.data.length,
        });
      }
    });
  };
  ...
  actionFormatter(cell, row, rowIndex, formatExtraData) {
    return (
      <div>
        <Button
            onClick={()=> this.editProductType(row)}
          color="outline"
          className="mt-n1"
        >
  ...
  confirmDeleteItem = () =>{
    ...
    this.props.deleteProductType(this.state.deleteID).then((item) => {
      if (item.status === 200) {
  ...
  handleUpdateFieldChange = (field, value) => {
    const newState = { ...this.state };
    newState.edit_type[field] = value;
    this.setState(newState);
  };
  ...
  handleUpdate = async (event) => {
    console.log(this.state)
    this.props
      .update(this.state.edit_type, this.state.edit_type._id)
      .then((resp) => {
        if (resp) {
          console.log(resp)
          if (resp.status === 200) {
    ...
    return (
      <Container fluid className="p-0">
        ...
        <Modal
          ...
          size="md"
        >
          <ModalHeader toggle={() => this.toggle()}>Add Product Type</ModalHeader>
          <AvForm
              ...
              <FormGroup row>
                ...
                <Col sm={8}>
                  <AvField
                    ...
                    onChange={(event) =>
                      this.handleFieldChange("productType", event.target.value)
                    }
                    validate={{
                      required: {
                        value: true,
        ...
        <Modal
          ...
          size="md"
        >
          <ModalHeader toggle={() => this.editToggle()}>Edit Product</ModalHeader>
          <AvForm
              ...
              <FormGroup row>
                ...
                <Col sm={8}>
                  <AvField
                    type="text"
                    name="productType"
                    value={this.state.edit_type?.productType}
                    onChange={(event) =>
                      this.handleUpdateFieldChange("productType", event.target.value)
                    }
                    validate={{
                      required: {
                        value: true,
        ...
        >
          ...
          <ModalBody className="text-center m-3">
            <p className="mb-0">Are you sure you want to delete this type?</p>
          </ModalBody>
          <ModalFooter>
        ...
        <h1 className="h3 mb-3">Types</h1>
        <UsersStaff tableColumns={this.tableColumns}  tableData={this.state.types}/>
      </Container>
    );
  }
}
...
const mapActionToProps = {
  create: productTypeActions.create,
  getProductTypes: productTypeActions.getProductTypes,
  update: productTypeActions.update,
